...
// --- tunables ---------------------------------------------------------------
const SEP = CARTRIDGE.world.duelSeparation || 1.7; // half the gap between the two marks
const PLAYER_X = -SEP, OPP_X = SEP;          // the two marks on the street
const HERO_SCALE = 0.78;
const TUNE = CARTRIDGE.tuning;
...
const OPP_REACTION_FLOOR = TUNE.opponentReactionFloor;       // …the final one is near-inhuman
const OPP_REACTION_STEP = TUNE.opponentReactionStep;         // faster each duel
const OPP_JITTER = TUNE.opponentJitter;

const opponentName = (i) => {
  const list = CARTRIDGE.opponents;
  return list[Math.min(i, list.length - 1)] + (i >= list.length ? ' +' + (i - list.length + 2) : '');
};
...
export function startGame({ canvas, hud }){
  ...
  scene.fog = new THREE.Fog(COLORS.fog, 22, 48);

  const FOV = 46, FRAME_HALF_W = Math.max(2.45, SEP + 1.2); // world half-width that MUST stay in frame
  ...
  const sky = new THREE.Mesh(
    ...
    new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        bot: { value: new THREE.Color(COLORS.skyBottom) }, glow: { value: new THREE.Color(COLORS.skyGlow) },
  ...
  scene.add(new THREE.HemisphereLight(COLORS.hemiSky, COLORS.hemiGround, 0.85));
  const key = new THREE.DirectionalLight(COLORS.keyLight, 1.25);
  ...
  scene.add(key);
  const rim = new THREE.DirectionalLight(COLORS.rimLight, 0.35);
  ...
  const ground = new THREE.Mesh(new THREE.PlaneGeometry(120, 120),
    new THREE.MeshStandardMaterial({ color: COLORS.ground, roughness: 1, flatShading: true }));
  ...
  function cactus(x, z, s){
    ...
    g.add(box(0.4 * s, 1.7 * s, 0.4 * s, COLORS.cactus, 0, 0.85 * s, 0, { r: 1 }));
    g.add(box(0.28 * s, 0.7 * s, 0.28 * s, COLORS.cactus, 0.36 * s, 1.1 * s, 0, { r: 1 }));
    g.add(box(0.28 * s, 0.55 * s, 0.28 * s, COLORS.cactus, -0.34 * s, 1.3 * s, 0, { r: 1 }));
    g.position.set(x, 0, z); g.traverse(o => { if (o.isMesh) o.castShadow = true; });
    return g;
  }
  function lantern(x, z, s){
    const g = new THREE.Group();
    g.add(box(0.12 * s, 1.6 * s, 0.12 * s, COLORS.townTrim, 0, 0.8 * s, 0, { r: 1 }));
    g.add(box(0.62 * s, 0.1 * s, 0.16 * s, COLORS.townTrim, 0, 1.52 * s, 0, { r: 1 }));
    g.add(box(0.22 * s, 0.34 * s, 0.22 * s, COLORS.sun, 0.24 * s, 1.28 * s, 0, { r: 1, e: COLORS.sun, ei: 0.8 }));
    g.position.set(x, 0, z); g.traverse(o => { if (o.isMesh) o.castShadow = true; });
    return g;
  }
  if (CARTRIDGE.world.props === 'dojo') {
    scene.add(lantern(-7, -4, 1.1)); scene.add(lantern(8, -6, 0.9)); scene.add(lantern(6.5, -2, 0.75));
  } else {
    scene.add(cactus(-7, -4, 1.1)); scene.add(cactus(8, -6, 0.9)); scene.add(cactus(6.5, -2, 0.6));
  }
  // backdrop silhouettes: the cartridge owns the world language, while the
  // engine keeps camera/framing fixed.
  const town = new THREE.Group();
  if (CARTRIDGE.world.backdrop === 'dojo-gate') {
    town.add(box(0.42, 4.2, 0.5, COLORS.town, -4.6, 2.1, -16, { r: 1 }));
    town.add(box(0.42, 4.2, 0.5, COLORS.town, 4.6, 2.1, -16, { r: 1 }));
    town.add(box(10.2, 0.5, 0.7, COLORS.townTrim, 0, 4.15, -16, { r: 1 }));
    town.add(box(8.6, 0.36, 0.7, COLORS.townTrim, 0, 3.45, -16, { r: 1 }));
    town.add(box(2.2, 1.2, 0.6, COLORS.town, -8.2, 0.6, -15.5, { r: 1 }));
    town.add(box(2.2, 1.2, 0.6, COLORS.town, 8.2, 0.6, -15.5, { r: 1 }));
    town.add(box(14, 0.32, 0.5, COLORS.townTrim, 0, 1.15, -17, { r: 1 }));
  } else {
    for (let i = -3; i <= 3; i++){
      if (i === 0) continue;
      const h = 2.2 + (i * 37 % 5) * 0.4;
      town.add(box(3.2, h, 2, COLORS.town, i * 4.2, h / 2, -16, { r: 1 }));
      town.add(box(3.6, 0.6, 2.2, COLORS.townTrim, i * 4.2, h + 0.2, -16, { r: 1 }));   // false-front cornice / roofline
    }
  }
  scene.add(town);
  ...
  const motes = new THREE.Points(mGeo, new THREE.PointsMaterial({ color: COLORS.motes, size: 0.07, transparent: true, opacity: 0.6, depthWrite: false, fog: false }));
  scene.add(motes);
  ...
  const playerRoster = CARTRIDGE.fighters.playerKeys || ROSTER;
  const opponentRoster = CARTRIDGE.fighters.opponentKeys || ROSTER;
  const pick = list => list[Math.floor(Math.random() * list.length)] || 'cowboy';
  ...
  function buildFighter(charKey, side){   // side -1 = player (left, faces +x), +1 = opp (right, faces -x)
    ...
    // cartridge action prop: revolver by default, blade for duel variants.
  ...
  function shatter(f){
    ...
    f.model.traverse(o => {
      ...
      spawnP(wp.x, wp.y, wp.z, col, slash
        ? {
            vx: dir * (0.65 + Math.random() * 1.15),
            vy: 0.9 + Math.random() * 1.8,
            vz: (Math.random() * 2 - 1) * 0.85,
            size: 0.14,
            life: 1.55,
            grav: 4.8,
            spin: 3.2,
  ...
  function sfxBlade(){
    noiseBurst(0.045, { gain: 0.28, hp: 2600 });
    tone(980, 0.12, { type: 'triangle', gain: 0.18, slideTo: 1720, lp: 4200 });
    tone(540, 0.16, { type: 'sine', gain: 0.08, slideTo: 240, delay: 0.08, lp: 1600 });
  }
  // a gunshot with texture: crack transient + low body thump + 2 canyon-echo slapbacks
  function sfxShot(){
    if (ACTION.sound === 'blade') { sfxBlade(); return; }
  ...
  function beginDuel(){
    ...
    hud.setWins(wins);
    hud.setOpponent(opponentName(duelIdx));
    hud.setStatus(COPY.wait, 'wait');
    hud.flashDraw(false);
  }
  ...
  function resolve(won, foul){
    ...
    hud.flashDraw(false);
    tenseUp(false);
    doSlow(ACTION.effect === 'slash' ? 0.18 : 0.28, ACTION.effect === 'slash' ? 1.25 : 0.95); // slow-mo covers the cinematic push-in
    if (foul){
      drawAndFire(opp); shatter(player);
      hud.setStatus(COPY.foul, 'foul');
      sfxFoul(); sfxLose();
    } else if (won){
  ...
  function reset(){
    ...
    player = buildFighter(pick(playerRoster), -1);
    setRestPose(player);
  ...
  function finalizeDeath(){
    state = DEAD;
    submitScore(wins);
    hud.setDead({ wins, best, killer: opponentName(duelIdx) });
  }
  ...
  function tick(now){
    ...
    if (state === SET){
      ...
      for (const f of [player, opp]){ if (f && f.flash && f.flash.material.opacity > 0){ f.flash.material.opacity = Math.max(0, f.flash.material.opacity - dt * (ACTION.effect === 'slash' ? 3.2 : 6)); f.flash.scale.multiplyScalar(1 + dt * (ACTION.effect === 'slash' ? 6 : 4)); } }
      if (resolveT > 1.3){
